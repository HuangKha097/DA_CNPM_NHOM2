...
const MessageTab = () => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      let res;
      const isSendToAll = targetRole === 'all_parents' || targetRole === 'all_drivers';
        ...
        <div className={cx('history-wrapper')}>
          ...
          <ul className={cx('history-list')}>
            {historyMessage.length === 0 ? (
              <li style={{ color: '#888', fontStyle: 'italic' }}>Chưa có lịch sử tin nhắn.</li>
            ) : (
      ...
      <div className={cx('right-block')}>
        <div className={cx('composer-wrapper')}>
          ...
          <form className={cx('composer-form')} onSubmit={handleSend}>
            ...
            {(targetRole === 'parent' || targetRole === 'driver') && recipientList.length > 0 && (
              <div className={cx('form-group')}>
            ...
            <button type="submit" className={cx('send-button')} disabled={isSending}>
