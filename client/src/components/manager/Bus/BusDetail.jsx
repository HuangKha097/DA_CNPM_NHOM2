...
const BusDetail = ({ busDetail, setBusDetail }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (res?.success) {
        if (driverId) {
          const driverRes = await UserController.fetchUserById(driverId);
      ...
      if (res?.success) {
        ...
        if (busDetail.students?.length > 0) {
          for (const student of busDetail.students) {
            await UserController.updateStudentAssignedBus(student._id, null);
          }
        }
  ...
  const findStudents = async () => {
    try {
      ...
      const response = await UserController.findStudents(true, searchValue);
                ...
                {isEditing ? (
                  <>
                    ...
                    {busDetail?.students?.length && (
                      <span>
                        {busDetail?.students?.length || studentsSelected.length}
                      </span>
                    )}
                  </>
                ) : (
                      ...
                      <button
                        ...
                        onClick={() => {
                          const newStudents = studentsSelected.filter(
                            (s) =>
                              !busUpdate.students.some((b) => b._id === s._id)
                          );
